...
export default function Home() {
  ...
  // Move browser detection to run first
  useEffect(() => {
    const detectBrowser = () => {
      ...
      console.log('[Debug] Browser detection:', { isChrome: !isMobile && isChromeBrowser });
    };

    detectBrowser();
  }, []);

  // Then handle background image loading
  useEffect(() => {
    if (!isChrome) {
      console.log('[Debug] Skipping background load - not Chrome');
      return;
    }

    console.log('[Debug] Starting background image load');
    const img = new window.Image();
    img.src = '/optimized/vebg-static.webp';
    img.onload = () => {
  ...
  // Update animation loading to depend on Chrome and background image
  useEffect(() => {
    const loadAnimation = async () => {
      if (!isChrome || !bgImageLoaded || animationLoadAttempted.current) {
        console.log('[Debug] Skipping animation load:', { 
          isChrome, 
          bgImageLoaded, 
          alreadyAttempted: animationLoadAttempted.current 
        });
        return;
      }
      
      console.log('[Debug] Starting animation load');
      ...
      try {
        ...
        const fetchPromise = fetch('/anim4k-opt2.json');
        ...
        console.log('[Debug] Animation JSON loaded');
        setAnimationData(data);
        
        // Wait for next frame to ensure animation data is processed
        requestAnimationFrame(() => {
          console.log('[Debug] Animation ready');
    ...
    loadAnimation();
  }, [isChrome, bgImageLoaded]);

  // Update the frame listener setup
  useEffect(() => {
    if (!lottieRef.current?.animationItem || initialAnimationComplete || !isAnimationReady || !isChrome) {
      console.log('[Debug] Frame listener setup skipped:', {
        hasAnimation: !!lottieRef.current?.animationItem,
    ...
    console.log('[Debug] Setting up frame listener');
    const handleFrame = () => {
      ...
      if (!hasReachedThirtyPercent.current && progress >= 0.3 && !hasScrolled) {
        console.log('[Debug] Pausing at 30% - No scroll');
      ...
      if (hasScrolled && !hasReachedThirtyPercent.current) {
        console.log('[Debug] Pausing - User scrolled before 30%');
        lottieRef.current?.animationItem?.pause();
        hasReachedThirtyPercent.current = true;
        setInitialAnimationComplete(true);
      }
  ...
  }, [initialAnimationComplete, hasScrolled, isAnimationReady, isChrome]);
  ...
  // Update the Lottie render function
  const renderLottieAnimation = () => {
    if (!animationData || !isChrome || !bgImageLoaded || !isAnimationReady) {
      console.log('[Debug] Animation render skipped:', {
        hasData: !!animationData,
        isChrome,
        bgImageLoaded,
        isAnimationReady
      });
        ...
        >
          ...
          >
            ...
            <div className="w-[90vw] sm:w-[80vw] md:w-[600px] flex justify-center mt-1vh [transition:transform_500ms_ease-out]"
              ...
              <Image
                src="/optimized/lauren david hero.svg"
                alt="Lauren & David"
                width={800}
                height={100}
